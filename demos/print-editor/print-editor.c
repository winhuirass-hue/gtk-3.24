...
load_file (GFile *open_filename)
{
  ...
  if (error == NULL)
	...
	{
          GFileInfo *info = g_file_query_info (open_filename, "standard::display-name", G_FILE_QUERY_INFO_NONE, NULL, &error);
...
save_file (GFile *save_filename)
{
  ...
  if (error == NULL)
    ...
    {
      GFileInfo *info = g_file_query_info (save_filename, "standard::display-name", G_FILE_QUERY_INFO_NONE, NULL, NULL);
      const char *display_name = g_file_info_get_display_name (info);
      GtkAlertDialog *alert;
