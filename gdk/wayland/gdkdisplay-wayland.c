...
#define ZXDG_DECORATION_VERSION         1
#define XDG_OUTPUT_VERSION              3
...
session_listener_created (void                  *data,
                          struct xdg_session_v1 *session_v1,
                          const char            *id)
{
  GdkWaylandDisplay *display_wayland = data;
                            ...
                            uint32_t            version)
{
  ...
  if (match_global (display_wayland, interface, version, wl_compositor_interface.name, 0))
    ...
    {
      display_wayland->session_manager =
        wl_registry_bind (display_wayland->wl_registry, id,
  ...
  else if (match_global (display_wayland, interface, version, zxdg_decoration_manager_v1_interface.name, 0))
    {
      display_wayland->zxdg_decoration_manager_v1 =
        wl_registry_bind (display_wayland->wl_registry, id,
                          &zxdg_decoration_manager_v1_interface, ZXDG_DECORATION_VERSION);
    }
...
gdk_wayland_display_dispose (GObject *object)
{
  ...
  g_clear_pointer (&display_wayland->zxdg_decoration_manager_v1, zxdg_decoration_manager_v1_destroy);
                                      ...
                                      const char                         *name)
{
  ...
  if (!display_wayland->session_manager)
    return FALSE;

  g_clear_pointer (&display_wayland->session_id, g_free);
  display_wayland->session_id = g_strdup (name);

  display_wayland->session =
    xdg_session_manager_v1_get_session (display_wayland->session_manager,
                                        reason,
                                        name);
  xdg_session_v1_add_listener (display_wayland->session,
                               &session_listener,
                               display_wayland);
...
gdk_wayland_display_unregister_session (GdkDisplay *display)
{
  ...
  if (display_wayland->session)
    xdg_session_v1_remove (display_wayland->session);
